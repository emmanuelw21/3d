...
function ThirdPersonControls({ avatarRef, speed = 0.1 }) {
  ...
  useFrame(() => {
    ...
    const rotatedOffset = offset.applyEuler(new Vector3(0, avatarRotation, 0));
  ...
  return (
    <Canvas style={{ height: '100vh', width: '100vw' }} backgroundColor="lightblue">
      ...
      <OrbitControls 
        ...
        enableRotate={true} 
        enableZoom={true} 
        target={[0, 1, 0]} // Initial target to focus the camera
        maxPolarAngle={Math.PI / 2} // Restrict the vertical angle
      />
    </Canvas>
  );
}
